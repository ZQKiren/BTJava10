...
//8
SELECT m.Name, SUM(DATEDIFF(b.ReturnDate, b.BorrowDate)) tongngaymuon
FROM Members m
JOIN Borrowings b ON b.MemberID = m.MemberID
...
//9
SELECT m.Name, COUNT(*)tongsosachmuon
FROM Members m
JOIN Borrowings b ON b.MemberID = m.MemberID
GROUP BY m.Name
LIMIT 1;
...
FROM Members m
JOIN Borrowings br ON br.MemberID = m.MemberID 
JOIN Books b ON b.BookID = br.BookID and b.Genre = 'Tiểu Thuyết'
GROUP BY m.Name
HAVING COUNT(*) > 2;
...
//12
SELECT b.Genre,COUNT(*) solanmuon
FROM Borrowings br
